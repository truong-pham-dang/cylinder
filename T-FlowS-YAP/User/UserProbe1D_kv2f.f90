...
!---- write the probe coordinates out
  ...
  allocate(var1(Nprob)); var1=0.0
  allocate(var2(Nprob)); var2=0.0
  if(HOT==YES) then
  ...
  do pl=1,Nprob
    do c=1,NC
      if( Approx( zc(c),z_p(pl) ) ) then
        ...
        vwp(pl)   = vwp(pl) + VISt(c)*abs(Uz(c)) 
!        vwp(pl)   = vwp(pl) + Cdyn(c) 
!        vwp(pl)   = vwp(pl) + Kin % mean(c) 
        VISt_m(pl)= VISt_m(pl) + Uf(c) 
        VISt_m1(pl)= VISt_m1(pl) + VISt_r(c) 
!        VISt_m2(pl)= VISt_m2(pl) + abs(uw % mean(c)-U % mean(c) * W % mean(c))/(abs(Uz(c))+tiny) 
        VISt_m2(pl)= VISt_m2(pl) + VISt(c)
        Kin_mp(pl)= Kin_mp(pl) + Kin % n(c)   
        Eps_mp(pl)= Eps_mp(pl) + Eps % n(c)   
        v_2_mp(pl)= v_2_mp(pl) + v_2 % n(c)   
        f22_mp(pl)= f22_mp(pl) + f22 % n(c)   
        Ksgsp(pl) = Ksgsp(pl) + Ksgs(c) 
        var2(pl)  = var2(pl) + Kin%mean(c)**1.5/(volume(c)**0.0333*0.01)
        if( Approx( xc(c),0.0490625 ) ) then
          if( Approx( yc(c),0.0490625 ) ) then 
            var1(pl)  = var1(pl) + U % n(c)
          end if
        end if
        if(HOT==YES) then
...
!---- average over all processors
  do pl=1,Nprob
    ...
    call GloSum(Kin_mp(pl))
    call GloSum(Eps_mp(pl))
    call GloSum(v_2_mp(pl))
    call GloSum(f22_mp(pl))
  ...
  do pl=1,Nprob
    ...
    Kin_mp(pl) = Kin_mp(pl) / Np(pl)
    Eps_mp(pl) = Eps_mp(pl) / Np(pl)
    v_2_mp(pl) = v_2_mp(pl) / Np(pl)
    f22_mp(pl) = f22_mp(pl) / Np(pl)
    if(HOT==YES) then
      Tmp(pl) = Tmp(pl) / Np(pl)
      TTp(pl) = TTp(pl) / Np(pl)
      uTp(pl) = uTp(pl) / Np(pl)
    ...
    do pl=1,Nprob
      if(HOT==YES) then
        ...
        write(9,'(20E17.7)')                   &
                       ...
                       uup(pl), vvp(pl), wwp(pl),  &
                       uvp(pl), uwp(pl), vwp(pl),  &
                       VISt_m(pl), VISt_m1(pl),VISt_m2(pl), & 
                       Kin_mp(pl), Eps_mp(pl), Ksgsp(pl), & 
                       v_2_mp(pl), f22_mp(pl), var1(pl), var2(pl) 
      end if
    end do

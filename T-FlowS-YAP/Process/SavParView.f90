...
!-------------------------------[Locals]-------------------------------!
  ...
  REAL    :: Utot, Unor, Utan, Apow, Bpow, nu, dely, yPlus, F_YAP
  REAL    :: frictionv(NC)
...
!--scalar: alpha field
  ...
  write(9,'(A62)') '        <DataArray type="Float32" Name="F_YAP" format="ascii">'
                             do c=1,NCsub
  F_YAP = (Kin % n(c) ** 1.5)/(Eps % n(c) + tiny)/(2.44*WallDs(c))
                                write(9,*) max(0.00,0.83*(Eps % n(c)**2) / Kin % n(c)*(F_YAP - 1)*(F_YAP**2)*volume(c))
                             end do
  write(9,'(A20)') '        </DataArray>'
...
!     write(*,*) 'writing parallel master file: ', name
  ...
  !--scalar: k and eps
     ...
     write(12,'(A48)') '        <PDataArray type="Float32" Name="F_YAP"/>'

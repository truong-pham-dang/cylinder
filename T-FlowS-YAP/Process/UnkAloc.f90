...
!======================================================================!
  ...
  !d/dr (U_phi) - used in TraceSeparationPoint.f90 for cylinder
  allocate (gradUphi_r(-NbC:NC)); gradUphi_r = 0.0
...
!---- variables for Rans models
  ...
  if(SIMULA==K_EPS_VV.or.SIMULA==ZETA.or.SIMULA==HYB_ZETA) then
    ...
    !!new: to compare with LES calculation
    !!mean Corsin scale
    allocate (Ls % mean(1:NC)); Ls % mean=0.
    !!mean turbulent viscosity
    allocate (VISt_mean(1:NC)); VISt_mean = 0.
    !!Second order mean covariances
    !!variable definition greatly differs here from ones in LES-test CalcBudgets_duct.f90!
    !normal
    ...
    !YAP
    allocate (grad_L_x(-NbC:NC));  grad_L_x=0.
    allocate (grad_L_y(-NbC:NC));  grad_L_y=0.
    allocate (grad_L_z(-NbC:NC));  grad_L_z=0.
  ...
  if(SIMULA==ZETA .or. SIMULA==HYB_ZETA) then
    !coherent components
    allocate (Ucoh0(-NbC:NC));  Ucoh0=0.0
    allocate (Vcoh0(-NbC:NC));  Vcoh0=0.0
    allocate (Wcoh0(-NbC:NC));  Wcoh0=0.0
    allocate (VIStcoh0(-NbC:NC));  VIStcoh0=0.0
    allocate (Vortcoh0(-NbC:NC));  Vortcoh0=0.0

    allocate (Ucoh13MIN(-NbC:NC));  Ucoh13MIN=0.0
    allocate (Vcoh13MIN(-NbC:NC));  Vcoh13MIN=0.0
    allocate (Wcoh13MIN(-NbC:NC));  Wcoh13MIN=0.0
    allocate (VIStcoh13MIN(-NbC:NC));  VIStcoh13MIN=0.0
    allocate (Vortcoh13MIN(-NbC:NC));  Vortcoh13MIN=0.0

    allocate (Ucoh23MIN(-NbC:NC));  Ucoh23MIN=0.0
    allocate (Vcoh23MIN(-NbC:NC));  Vcoh23MIN=0.0
    allocate (Wcoh23MIN(-NbC:NC));  Wcoh23MIN=0.0
    allocate (VIStcoh23MIN(-NbC:NC));  VIStcoh23MIN=0.0
    allocate (Vortcoh23MIN(-NbC:NC));  Vortcoh23MIN=0.0

    allocate (UcohMIN(-NbC:NC));  UcohMIN=0.0
    allocate (VcohMIN(-NbC:NC));  VcohMIN=0.0
    allocate (WcohMIN(-NbC:NC));  WcohMIN=0.0
    allocate (VIStcohMIN(-NbC:NC));  VIStcohMIN=0.0
    allocate (VortcohMIN(-NbC:NC));  VortcohMIN=0.0

    allocate (UcohMINCanditate(-NbC:NC));  UcohMINCanditate=0.0
    allocate (VcohMINCanditate(-NbC:NC));  VcohMINCanditate=0.0
    allocate (WcohMINCanditate(-NbC:NC));  WcohMINCanditate=0.0
    allocate (VIStcohMINCanditate(-NbC:NC));  VIStcohMINCanditate=0.0
    allocate (VortcohMINCanditate(-NbC:NC));  VortcohMINCanditate=0.0
    ...
    !Reynolds stresses total
    !normal
    !allocate (uu_total(1:NC)); uu_total = 0.
    !allocate (vv_total(1:NC)); vv_total = 0.
    !allocate (ww_total(1:NC)); ww_total = 0.
    !!shear
    !allocate (uv_total(1:NC)); uv_total = 0.
    !allocate (uw_total(1:NC)); uw_total = 0.
    !allocate (vw_total(1:NC)); vw_total = 0.
    !!Total turbulent viscosity
    !allocate (VISt_total(1:NC)); VISt_total=0.
    !allocate (ShearMean(1:NC)); ShearMean=0.

    !partial derivatives 1st order of mean velocities 
    !allocate(grad_U_x(1:NC)); grad_U_x(:) = 0.0
    !allocate(grad_U_y(1:NC)); grad_U_y(:) = 0.0
    !allocate(grad_U_z(1:NC)); grad_U_z(:) = 0.0
    !allocate(grad_V_x(1:NC)); grad_V_x(:) = 0.0
    !allocate(grad_V_y(1:NC)); grad_V_y(:) = 0.0
    !allocate(grad_V_z(1:NC)); grad_V_z(:) = 0.0
    !allocate(grad_W_x(1:NC)); grad_W_x(:) = 0.0
    !allocate(grad_W_y(1:NC)); grad_W_y(:) = 0.0
    !allocate(grad_W_z(1:NC)); grad_W_z(:) = 0.0
  end if
  ...
  if(SIMULA == HYB_ZETA .or. SIMULA==ZETA) then
    ...
    allocate (VISt_sgs_mean(-NbC:NC));  VISt_sgs_mean=0.
    allocate (VISt_eff(-NbC:NC));  VISt_eff=0.
    allocate (VISt_eff_mean(-NbC:NC));  VISt_eff_mean=0.
  end if      
...
!---- variables defined in les_mod.h90:
  ...
  if(SIMULA == LES.or.SIMULA==DNS.or.SIMULA==DES_SPA) then
    ...
    allocate (Eps % mean(NC));   Eps % mean=0.
    allocate (Ls % mean(1:NC)); Ls % mean=0.
    allocate (VISt_mean(NC)); VISt_mean = 0.

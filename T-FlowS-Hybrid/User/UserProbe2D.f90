...
!-------------------------------[Locals]-------------------------------!
  ...
  REAL                :: u_tau, Re_tau, tau_w,u_tau_i,tau_w_i, Uref, WD, Ge, teta
...
!---- write the values at the probes
    ...
    if(SIMULA==SPA_ALL.or.SIMULA==DES_SPA) then
      ...
      if(HOT == YES) then
        do pl=1,Npoints
          z_p(pl)   =  (z_p(pl))*VISc/u_tau
          Ge = (0.01*(0.71*z_p(pl)*2000)**4.0)/(1.0+5.0*0.71**3.0*z_p(pl)*2000)
          teta = 0.71*z_p(pl)*2000*exp(-Ge) + ( 2.12*log((1.0+z_p(pl)*2000)    &  
                 * (1.5*2.0 - 1.5*z_p(pl))/(1.0+2.0*(1.0-z_p(pl))**2.0) ) + 3.83  )*exp(-1.0/Ge)
          write(9,'(9F12.6)')                      &
                       z_p(pl)*u_tau/VISc,                    &
                       Ump(pl), Vmp(pl), Wmp(pl), T_mp(pl), VISt_mp(pl),VIS_mp(pl), uvp(pl), teta
        end do
      else 
        do pl=1,Npoints
          write(9,'(7F12.6)')                      &
...
!    end if
    if(SIMULA==K_EPS_VV.or.SIMULA==LES.or.SIMULA==ZETA) then
      ...
      if(HOT == YES) then
        do pl=1,Npoints
          Ge = (0.01*(0.71*z_p(pl))**4.0)/(1.0+5.0*0.71**3.0*z_p(pl))
          teta = 0.71*z_p(pl)*exp(-Ge) + ( 2.12*log((1.0+z_p(pl))    &  
                 * (1.5*2.0 - 1.5*z_p(pl)*VISc/u_tau)/(1.0+2.0*(1.0-z_p(pl)*VISc/u_tau)**2.0) ) + 3.83  )*exp(-1.0/Ge)
          write(9,'(12F16.8)')                      &
                       z_p(pl),                    &
                       Ump(pl), Vmp(pl), Wmp(pl), T_mp(pl), &
                       Kin_mp(pl), Eps_mp(pl),     &
                       uvp(pl), v_2_mp(pl),        &
                       f22_mp(pl), VISt_mp(pl), teta
        end do
      else
        do pl=1,Npoints 
          write(9,'(11F16.8)')                      &
